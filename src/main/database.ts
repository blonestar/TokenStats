...
export type UsageEvent = TokenUsage & { eventId: string; sourceId: string; sessionId: string; occurredAt: string; relativeFile: string; byteOffset: number; parserVersion: string; model: string; included?: boolean }
...
INSERT OR IGNORE INTO schema_migrations(version, applied_at) VALUES(1, datetime('now'));`)
  ...
  deactivateCopilotOtel(sourceId: string, relativeFile: string): number {
  ...
  reconcileCopilotOtel(sourceId: string, relativeFile: string): { removedFallback: number; activeEvents: number; completeEvents: number } {
    const pairs = this.db.prepare('SELECT DISTINCT session_id sessionId,coalesce(model,\'Unknown\') model FROM usage_events WHERE source_id=? AND relative_file=?').all(sourceId, relativeFile) as Array<{ sessionId: string; model: string }>
