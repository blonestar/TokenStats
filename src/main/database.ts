...
INSERT OR IGNORE INTO schema_migrations(version, applied_at) VALUES(1, datetime('now'));`)
    ...
    const hasV7 = this.db.prepare('SELECT 1 FROM schema_migrations WHERE version=7').get()
    if (!hasV7) this.db.transaction(() => {
      this.db.exec('ALTER TABLE usage_events ADD COLUMN cache_write_one_hour_input_tokens INTEGER')
      this.db.prepare("INSERT INTO schema_migrations(version, applied_at) VALUES(7, datetime('now'))").run()
    })()
    this.db.transaction(() => {
  ...
  reconcileSnapshot(sourceId: string, relativeFile: string, cursor: number, events: UsageEvent[]): number {
    ...
    return this.db.transaction(() => { let inserted = 0; for (const event of events) { const values = { ...event, cacheWriteOneHourInputTokens: event.cacheWriteOneHourInputTokens ?? null, included: event.included === false ? 0 : 1, insertedAt: new Date().toISOString() }; const changed = insert.run(values).changes; inserted += changed; if (changed === 0) update.run(values) } stale.run(sourceId, relativeFile, events[0].sessionId, ...ids); this.db.prepare('INSERT INTO source_cursors VALUES(?,?,?) ON CONFLICT(source_id,relative_file) DO UPDATE SET byte_offset=excluded.byte_offset').run(sourceId, relativeFile, cursor); return inserted })()
  }
  ...
  dashboard(periodInput: unknown = 'thisMonth', now = new Date()): Dashboard {
    ...
    const costEvents = this.db.prepare('SELECT source_id sourceId,coalesce(model,\'Unknown\') model,input_tokens inputTokens,output_tokens outputTokens,cached_input_tokens cachedInputTokens,cache_write_input_tokens cacheWriteInputTokens,cache_write_one_hour_input_tokens cacheWriteOneHourInputTokens,reasoning_output_tokens reasoningOutputTokens FROM usage_events WHERE included=1 AND occurred_at>=? AND occurred_at<?').all(...params) as Array<Pick<UsageEvent, 'sourceId' | 'model' | 'inputTokens' | 'outputTokens' | 'cachedInputTokens' | 'cacheWriteInputTokens' | 'cacheWriteOneHourInputTokens' | 'reasoningOutputTokens'>>
  ...
  traySummary(now = new Date()): TraySummary {
    ...
    const costIn = (range: DashboardRange): CostEstimate => {
      const events = this.db.prepare('SELECT source_id sourceId,coalesce(model,\'Unknown\') model,input_tokens inputTokens,output_tokens outputTokens,cached_input_tokens cachedInputTokens,cache_write_input_tokens cacheWriteInputTokens,cache_write_one_hour_input_tokens cacheWriteOneHourInputTokens,reasoning_output_tokens reasoningOutputTokens FROM usage_events WHERE included=1 AND occurred_at>=? AND occurred_at<?').all(range.start, range.end) as Array<Pick<UsageEvent, 'sourceId' | 'model' | 'inputTokens' | 'outputTokens' | 'cachedInputTokens' | 'cacheWriteInputTokens' | 'cacheWriteOneHourInputTokens' | 'reasoningOutputTokens'>>
