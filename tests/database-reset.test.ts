...
describe('database reset support', () => {
  it('creates a verified backup and clears imported data while keeping the schema', async () => {
    ...
    expect(JSON.parse(readFileSync(`${backup}.json`, 'utf8'))).toMatchObject({ format: 'tokenstats-database-backup', appVersion: 'test-version', schemaVersion: 7, dataCounts: counts })
    ...
    expect(database.db.prepare('SELECT count(*) FROM schema_migrations').pluck().get()).toBe(7)
