...
describe('multi-source orchestration', () => {
  it('uses only absolute source overrides and otherwise keeps current-user defaults', () => { const sources = currentSources('/current-user', { CLAUDE_CONFIG_DIR: 'relative-config', COPILOT_HOME: '/copilot-home' } as NodeJS.ProcessEnv); expect(sources.map((source) => source.root)).toEqual([join('/current-user', '.codex', 'sessions'), join('/current-user', '.claude', 'projects'), join('/copilot-home', 'session-state')]); expect(sourceRoot('', '/fallback')).toBe('/fallback') })
